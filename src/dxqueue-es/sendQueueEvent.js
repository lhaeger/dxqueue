...
function sendQueueEvent(driverDN, myElement) {
    ...
    if (!(Packages.com.novell.nds.dirxml.engine.DirXML.isExternal())) {
        try {
            ...
            output = NdsDtd.createStatusDocument((new java.lang.Integer(0)), null, 'Submitted document for execution on subscriber channel on driver: ' + currentDriverDN)
                .getDocumentElement()

        } catch (t) {
            return NdsDtd.createStatusDocument((new java.lang.Integer(0)), opdata, 'Error: ' + t).getDocumentElement()
